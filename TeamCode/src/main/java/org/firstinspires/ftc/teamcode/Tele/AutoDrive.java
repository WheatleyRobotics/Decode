...
public class AutoDrive extends OpMode {
    ...
    public void loop() {
        ...
        if (camPose != null) {
            follower.setPose(lastCurrentLimeLightPos);
            lastCurrentLimeLightPos = camPose;
        }

        if (gamepad1.b) {
            Pose current = limelight.getLastPose(); // use the last valid Limelight pose
            limelight.setPose(new Pose(
                    current.getX(),               // keep current X
                    current.getY(),               // keep current Y
                    Math.toRadians(gyroPos)       // set new heading from gyro
            ));
        }
